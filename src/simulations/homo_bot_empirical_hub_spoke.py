# We plot average ergodic sum versus for a homogeneous hub and spoke network equipped with a bot
# a bot with weight beta and strength eta is connected ONLY to the hub (first node of the network)
...
A = np.load("data/A_hub_spoke.npy")
...
for horizon in range(3, Time + 3):
    ...
    for sim in range(5000):
        ...
        X_history, Z_history = simulate_trajectory_bot_one(
            N=N,
            horizon=horizon,
            T=T,
            L=L,
...
np.save(
    "data/all_means_N10_M5_homo_hub_spoke_bot_.npy",all_means
)
